(function(){
  ...
  const cfg=(()=>{
    ...
    return{h:el?.dataset?.heading||'Shipping Method',st:el?.dataset?.standardTitle||'Standard Shipping',sp:el?.dataset?.standardPrice||'Free',sd:el?.dataset?.standardDetail||'5-7 business days',pt:el?.dataset?.premiumTitle||'Premium Shipping with INK',pp:el?.dataset?.premiumPrice||'$4.99',f1:el?.dataset?.premiumFeature1||'📦 2-day delivery',f2:el?.dataset?.premiumFeature2||'📋 Return Passport for easy returns',vid:el?.dataset?.premiumVariantId||''};
  })();
